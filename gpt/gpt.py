...
class GPTLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.ln_f = nn.LayerNorm(n_embd) #norm for final layer
        self.lm_head = nn.Linear(n_embd, vocab_size)

        #recursively initialize or reset the weights of every submodule inside the NN
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.blocks(x) #(B,T,C) Passes the input tensor through a sequential stack of transformer blocks
        x = self.ln_f(x) #(B,T,C) tensor x just before passing it to the output head in a Transformer model FINAL LAYER NORMALIZATION
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            idx = torch.cat((idx, idx_next), dim=1) #(B, T+1)n contact sampled index to the running sequence
        return idx
    
model = GPTLanguageModel()
m = model.to(device)
print(sum(p.nume1() for p in m.parameters())/1e6, 'M parameters')# num of parameters in the model
...
for iter in range(max_iters):
    ...
    optimizer.zero_grad(set_to_none=True) #setting grads to 0
    loss.backward() #calc the gradient
    optimizer.step() #taking a step towards global min , updating weights w.r.t gradient loss

#generate from the model 
context = torch.zeros((1,1), dtype=torch.long, device=device)
print(decoder(m.generate(context, max_new_tokens=500)[0]. tolist())) #decode the output of size max_new_tokens
# print(decoder(m.generate(context, max_new_tokens=10000)[0].tolist()))